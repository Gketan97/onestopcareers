...
### Job description: boilerplate stripped, structured sections, collapsed by default (2026-08-26)

A user complaint ("too much content, not formatted well") turned into a real look at what was actually being shown — a real Razorpay job posting sample revealed the old flat-text renderer was showing 100% of the raw HTML content, of which maybe 20% was actually about the role. The rest was company boilerplate: founding story, funding round history ("$740 million raised"), transaction-volume marketing copy, equal-employment legal text, "follow us on LinkedIn." Brainstormed five options with the user first (fix formatting only / collapse-only / structural extraction / AI summarization / drop the feature entirely) and got explicit alignment on combining structural extraction with a real collapse, before writing any code — including naming AI summarization's accuracy risk directly as a reason not to default to it.

**`netlify/functions/job-description.js`'s old flat `htmlToText()` replaced with a real structured parser** (`parseDescription()`) that returns clean `{ heading, paragraph, items }[]` sections instead of a text blob, with company-boilerplate content stripped via pattern matching (funding mentions, "equal opportunity employer," "follow us on," founding-story language). Deliberately returns structured data, never raw HTML — the frontend renders it with normal JSX, never `dangerouslySetInnerHTML`, sidestepping any HTML-injection risk entirely rather than sanitizing and managing it.

**Two real bugs caught by testing against an actual sample before shipping, not assumed away:**
1. The real HTML had `<br>` tags sitting *inside* `<strong>` tags (`<strong>About the Role:<br></strong>`) — this split the heading-detection sentinel marker across two separate lines during parsing, breaking heading detection entirely. First test run against the real sample returned an empty array; traced it back to this specific pattern rather than guessing.
2. A trailing unheaded boilerplate paragraph (the "equal opportunity employer" line) was silently attaching itself to whichever section happened to be open at that point in the document — causing the combined-text boilerplate check to wrongly discard that *entire* section, including its legitimate bullet items, not just the boilerplate sentence. Caught by printing every intermediate parsed line and section object during testing, not by reading the code and assuming it was correct.

Lever's fetcher was also rewritten to feed its already-labeled `lists` (e.g. "Requirements") into the same `parseDescription()` used by every other source, rather than maintaining separate bespoke section-building logic for one source.

**Frontend (`DescriptionSections.tsx`):** renders each section as real headings/paragraphs/bullet lists. Collapse behavior measures the actual rendered DOM height (`useLayoutEffect` + a ref) against a threshold, rather than guessing from section or item counts — content length varies too much (a two-line "About the Role" vs. a long requirements list) for a count-based heuristic to be reliable. The "Show more" affordance only appears when content genuinely overflows; short, already-brief postings show fully with no collapse UI at all.

### Automatic jsDelivr cache purge added to the crawl workflow (2026-08-26)

After the Google Sheet parsing bug was fixed and confirmed (real evidence: `Sources: { greenhouse: 416, lever: 82, manual: 1 }` in the crawler's own log, plus a successful push shown in the same log), the job still didn't appear on the live site for a while — not a code bug this time, but jsDelivr's caching for `@main`-branch-referenced files not always picking up a fresh commit immediately. Manually visiting `purge.jsdelivr.net` with the file's URL fixed it immediately, confirmed working.

**Turned into a permanent fix rather than a one-off workaround:** added a `Purge jsDelivr cache` step to `.github/workflows/crawl.yml`, running right after the commit+push step, hitting jsDelivr's purge endpoint for both `jobs.json` and `meta.json` via `curl`. Uses `${{ github.repository }}` (a built-in GitHub Actions context value) rather than hardcoding the repo name, so this doesn't quietly break if the repo is ever renamed. Runs unconditionally on every crawl — including runs where nothing actually changed and no commit happened — a deliberate simplicity choice: an unneeded purge is harmless (jsDelivr just re-fetches identical content), while trying to conditionally skip it by threading a "did we commit" flag between steps adds real complexity for close to zero benefit, and getting that condition wrong would silently reintroduce the exact bug this step exists to eliminate. Every future crawl now reflects on the live site within moments of pushing, with no manual purge step required ever again.
...
**A real limitation worth naming plainly: this sandbox could not directly verify the new URL returns real data before shipping it.** Tried three methods — jsDelivr directly (blocked by this sandbox's own network allowlist, `jsdelivr.net` isn't on it), `raw.githubusercontent.com` (404), and `codeload.github.com` (404 on the whole repo tarball). Initially read the `raw.githubusercontent.com` 404 as "the file doesn't exist," which would have been a false alarm — caught and corrected before it became one: GitHub returns 404 (not 403) for private repos accessed without authentication, specifically so it doesn't confirm private repos exist at all. Given `Gketan97/onestopcareers` is almost certainly private (real commits have been pushed to it throughout this project), that's the far more likely explanation than the file being missing, especially given independent real evidence from the user's own terminal output earlier in this project showing genuine successful crawl commits (`chore: crawl incremental — 496 jobs [skip ci]`) landing on this exact repo. Asked the user to confirm the URL directly in their own browser rather than either asserting false confidence or asserting a false alarm from an ambiguous signal.

**Also fixed in the same file, from the same audit (OSC-005):** `fetchJobs()` had no timeout at all — the single most-depended-on fetch in the entire app (every page needs it) could hang indefinitely on a slow CDN response. Added `AbortSignal.timeout(10000)`, matching the pattern already used in `fetchJobDescription.ts`.

**Explicit next step, not yet done:** once the new CDN URL is confirmed showing real jobs on the live site, disable the old `jobscout-date` repo's GitHub Action — doing this *before* confirming the cutover would freeze the site on stale data permanently, since jsDelivr just keeps serving whatever was last committed.
...
**What shipped instead:** a central hub with four satellite nodes connected by spokes — a direct translation of `ServicesConstellation.tsx`'s actual real structure (one platform, four real services: Discover/Build/Connect/Prepare) into the mark itself, not an invented metaphor. This is now the **single canonical brand mark**, replacing the italic "o" everywhere: `public/favicon.svg` was rewritten to this design (verified legible at real favicon sizes — 16px and 32px renders were actually checked, not assumed), and the OAuth consent screen logo is rendered directly from that same SVG file at 512×512, so the favicon and the Google-facing logo can never drift apart into two different marks — one file is the source of truth for both.

### PostHog config matched to the real project (2026-08-24)

Once a real PostHog project existed, the user shared its actual dashboard-generated init snippet. `initAnalytics()` in `posthog.ts` was updated to match its exact config — `defaults: '2026-05-30'` (pins PostHog's own baseline behavior to the version their dashboard generated for this project, so it won't silently change if PostHog updates their overall recommended defaults later) and `person_profiles: 'identified_only'` (no profile created for anonymous browsing, only once someone actually authenticates via `identifyUser()`). Both options were confirmed to type-check cleanly against the installed `posthog-js` package's real TypeScript definitions — genuine confirmation these are valid current options, not something taken on faith from the dashboard snippet alone. The existing explicit `capture_pageview`/`autocapture`/`persistence` settings were kept as-is; they coexist with `defaults` without conflict, since explicitly-passed keys override the dated baseline.

Real credentials now live in Netlify's environment variables (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_POSTHOG_KEY`, `VITE_POSTHOG_HOST`) — this is the first point in the project where the backend scaffolding built in phase 1 has real values behind it, rather than being entirely untested.

**Real issues hit during first live setup, both configuration gaps rather than code bugs:** (1) Google OAuth login failed with `"provider is not enabled"` — expected, since the Google Cloud OAuth client + Supabase provider toggle hadn't been set up yet; email/password was used to validate the rest of the pipeline (schema, RLS, the profile-creation trigger) independently while that gets configured. (2) Email confirmation links redirected to `http://localhost:3000` and failed — Supabase's Auth "Site URL" was still its stock default value (notably not even matching this project's actual dev port, which is Vite's `5173`, not `3000` — a clear sign it had simply never been changed), not the live site's real URL. Fixed by updating Site URL and the Redirect URLs allowlist in Supabase's Auth settings to the real production domain (plus `localhost:5173` for local dev testing going forward).

### Backend phase 1 — Supabase + PostHog foundation (2026-08-23)

The first real backend this project has ever had. Everything before this point was a static frontend consuming a CDN-published JSON feed plus one small serverless function. This is a genuinely new category of infrastructure, not an extension of the existing pattern.

**Architecture decision, made explicitly before writing any code:** the requested schema included `companies` and `jobs` tables with `jobs.company_id → companies.id`. Decided NOT to mirror the crawler's job data into Supabase — doing so would require building a real sync pipeline (crawler → Supabase), which is exactly the "unnecessary backend infrastructure" the brief itself said to avoid. Instead: `saved_jobs.job_id` is plain text, matching the crawler's own id format (e.g. `gh-12345`) with no foreign key constraint. The `companies` table was dropped entirely once this was settled — it only existed in the original ask to support the FK relationship being skipped. `/jobs`, `/companies`, and `/companies/:slug` are completely unchanged by this phase; they still read from the CDN exactly as before.

**Tables shipped:** `profiles` (auto-created on signup via a trigger), `saved_jobs` (RLS: users see/insert/delete only their own rows), `career_circles` (a lookup table of circle types, public read, seeded with one row for the existing analytics circle), `career_circle_interests` (public insert requiring `consent_to_contact = true`, admin-only read/update via an `is_admin` flag on `profiles`).

**Three real bugs caught before shipping, not after:**
1. `CREATE TYPE ... AS ENUM` does not support `IF NOT EXISTS` in Postgres (unlike `CREATE TABLE`) — the schema file's own header claimed "safe to re-run," which would have been false on a second run without a `DO` block exception-handling wrapper around the enum creation. Caught by re-reading the SQL carefully, since this sandbox has no Postgres available to actually execute it against.
2. The first draft of `AuthContext.tsx` had a comment claiming `login_completed` would fire for Google OAuth "from the listener once the user returns" — but the listener never actually called it. Fixed using the real Supabase auth event type (`event === 'SIGNED_IN'`) combined with `session.user.app_metadata.provider === 'google'`, specifically to avoid double-firing against the email/password path's own explicit call at its call site.
3. `import.meta.env` had never been typed in this project (`src/vite-env.d.ts` didn't exist) — a real pre-existing gap that simply never surfaced because no code had ever used an env variable before this phase. Fixed by adding the missing `/// <reference types="vite/client" />` file.

**A real performance regression, caught and fixed in the same pass:** adding `@supabase/supabase-js` and `posthog-js` as plain imports roughly doubled the production bundle (245KB → 503KB), triggering Vite's own oversized-chunk warning. Fixed with `manualChunks` in `vite.config.ts`, splitting both into a separate `vendor_backend` chunk — the main app bundle is back to its original size, and the backend chunk now caches independently (and changes far less often than app code, so users re-download it far less frequently once cached).

**Analytics:** the exact requested event set is defined in `src/lib/analytics/posthog.ts`, typed so that PII (email, name, phone) can't be passed to any event function without fighting the type signatures — not just a documented convention. `company_id`/`circle_id` use real values where they exist (Career Circle events use the real Supabase UUID); job/company events use `company` (the name) instead of a `company_id`, since no such id exists anywhere in the data model per the architecture decision above. Search-tracking events (`job_search`, `companies_search`) are debounced 600ms after typing stops, not fired per keystroke — the search inputs themselves intentionally still filter instantly on every keystroke (a UX decision, unchanged), only the *tracking* is debounced.

**Wired end-to-end:** Saved Jobs (button on both `JobCard` variants and `JobDetail`, a `/saved` page, RLS-protected), Login (`/login`, Google + email/password, redirects via `?next=`), the Career Circle interest form (`/career-circle/join`, no account required, honest confirmation copy that explicitly says review is manual and nothing is automatic), and an auth-aware `Nav` (real "Log in" link / "Saved" + logout, replacing the permanently-inert placeholder that existed since the v7 rebuild). `job_viewed`, `job_apply_clicked`, `job_search`, `job_filter_applied`, `company_viewed`, `companies_search`, `career_circle_viewed`, `career_circle_interest_started`, `career_circle_interest_submitted`, `login_completed`, `signup_completed` are all actually firing from real UI, not just defined.

**Not done yet, explicitly:** `company_job_clicked` is defined in `posthog.ts` but not called from anywhere — would need a callback prop threaded into `JobCard` specifically for the company-profile-page context, not done in this pass. The minimal admin view for Career Circle submissions (list, status update) hasn't been started. **Nothing in this phase has been tested against real Supabase/PostHog credentials** — no project existed to test against during this build; the code is written carefully against the well-documented, stable public SDK APIs for both services, but "carefully written" is not the same guarantee as "confirmed working," and the honest status is: pending a real end-to-end smoke test once credentials exist.
